...
class TestIntegrationSprint2(unittest.TestCase):
    ...
    def test_password_change_integration(self):
        ...
        for entry in entries:
            ...
            decrypted = crypto.decrypt(entry['encrypted_data'], old_key)
            # Encrypt with new key
            new_encrypted = crypto.encrypt(decrypted, new_key)
            # Update in database
            self.db.update_entry(entry['id'], encrypted_data=new_encrypted)

        # 3. Update auth hash and salt
        ...
        for entry in updated_entries:
            #Проверяем encrypted_data
            decrypted = crypto.decrypt(entry['encrypted_data'], new_encryption_key)
